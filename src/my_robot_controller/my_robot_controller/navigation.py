...
class TurtleNavigationNode(Node):
    ...
    def __init__(self):
       ...
       # Initialize goal poses as dictionaries {x, y, w}
        ...
        self.goal_poses.append({'x': 0.0, 'y': -1.5, 'yaw': -30})
        self.goal_poses.append({'x': 6.5, 'y': -1.67, 'yaw': 60})
        self.goal_poses.append({'x': 6.5, 'y': -0.37, 'yaw': 40})
        self.goal_poses.append({'x': 1.8, 'y': -0.0, 'yaw': 0})

        self.goal_poses.append({'x': 6.0, 'y': 1.3, 'yaw': -30})
        self.goal_poses.append({'x': 2.0, 'y': 2.5, 'yaw': 60})
        self.goal_poses.append({'x': 6.0, 'y': 3.1, 'yaw': 40})
        self.goal_poses.append({'x': 1.7, 'y': 2.1, 'yaw': 0})
